...
X_train_seq_pad = tf.keras.preprocessing.sequence.pad_sequences(X_train_seq,
                                                                maxlen=MAX_WORDS,
                                                                padding="post",
                                                                truncating="post")
X_test_seq_pad = tf.keras.preprocessing.sequence.pad_sequences(X_test_seq,
                                                               maxlen=MAX_WORDS,
                                                               padding="post",
                                                               truncating="post")

model = tf.keras.models.Sequential([
    tf.keras.layers.Embedding(input_dim=VOCAB_SIZE, output_dim=16, input_length=MAX_WORDS),
    tf.keras.layers.LSTM(MAX_WORDS, input_shape=(None, 16)),
