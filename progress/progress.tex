...
Our team does not wish to have rigidly-defined roles. We each have an interest in collaborating on each phase of the project.

Wesley shows the strongest interest in coding the neural network, so he will be the main programmer on that, and Miles and Mauricio will contribute to this phase as they desire. This task involves training the network with hate speech data and testing it. The neural network will be what the bot uses to label messages as hate speech or not hate speech.

Miles would like to integrate the neural network into an online hate speech detection service that he will code and test. The service will be used by the Discord bot to do its message analysis and hate speech detection. 

Mauricio would like to incorporate the online hate speech detection service into a Discord server bot that he will code and test. The bot will detect hate speech in Discord server messages in real time.

Everyone will work on the write-ups and presentation, and everyone will help to find more Twitter data that we can use to improve upon the neural network.
